...
| 기능                   | 설명                                                                  |
|----------------------|---------------------------------------------------------------------|
| **🤖 AI 게임 데이터 생성**  | Claude Sonnet 4가 테마와 난이도에 맞는 독창적인 퍼즐과 스토리와 게임 로직이 포함된 C# 스크립트 자동 생성 |
| **🎨 3D 모델 자동 생성**   | Meshy AI를 통한 게임 오브젝트 3D 모델 실시간 생성                                   |
| **💻 Unity 스크립트 로드** | AI로 제작한 C# 스크립트를 syntax 오류 체크 및 필요 컴포넌트 부착, 런타임 컴파일 기능              |
...
> * 방탈출 게임 하나를 만드는데 **평균 3-6개월** 소요
> * 기획자, 개발자, 3D 디자이너 등 **기본 5명이상의 전문가** 필요
> * 개발 비용 **수천만원** 이상

### 💡 **ERoom의 혁신**

> * 방탈출 게임 생성 시간: **3-6개월 → 5분** (99.9% 단축)
> * 필요 인력: **전문가 팀 → 일반 사용자 1명** (커스터마이즈 )
> * 생성 비용: **게임 개발에 들어가는 수천만원 → API 사용료** (약 99% 절감)
...
> * **Undertow HTTP 서버**: 고성능 NIO 기반 웹 서버
> * **비동기 큐 시스템**: 다중 AI 요청의 안정적인 순차 처리 (최대 동시 처리: 1) *추후에 늘릴 예정
...
3. **AI 처리**:
    ...
    - Unity6 C# 스크립트 생성 (3-5분)
    - Meshy AI로 3D 모델 병렬 생성 (3-6분)
...
> * **Meshy AI** - Text-to-3D API 제공 플랫폼 (v2 API, meshy-4 모델)
> * **Shap-e** - Text-to-3D 생성형 모델 (hugging-face)
> * **Unity6** - 최신 게임 엔진
